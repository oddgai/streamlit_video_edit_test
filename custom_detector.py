...
class BlackWhiteThresholdDetector(SceneDetector):
    """真っ黒/真っ白の画面を検出する
	デフォルトのthreshold_detector.py を一部変更したもの
    ...
    """
    def __init__(self, black_threshold=12, white_threshold=244, min_percent=0.95, min_scene_len=15,
                 fade_bias=0.0, add_final_scene=False, block_size=8):
        """Initializes threshold-based scene detector object."""

        super(BlackWhiteThresholdDetector, self).__init__()
    ...
    def frame_under_threshold(self, frame):
        ...
        while curr_frame_row < frame.shape[0]:
            ...
            if large_ratio:
                black_frame_amt = int(np.sum(block > self.black_threshold))
                white_frame_amt = int(np.sum(block < self.white_threshold))
                if black_frame_amt < white_frame_amt:
                    curr_frame_amt += black_frame_amt
                else:
                    curr_frame_amt += white_frame_amt
            else:
                black_frame_amt = int(np.sum(block <= self.black_threshold))
                white_frame_amt = int(np.sum(block >= self.white_threshold))
                if black_frame_amt < white_frame_amt:
    ...
    def process_frame(self, frame_num, frame_img):
        # type: (int, Optional[np.ndarray]) -> List[int]
        """
        Args:
            frame_num (int): Frame number of frame that is being passed.
            frame_img (np.ndarray or None): Decoded frame image (np.ndarray) to perform
